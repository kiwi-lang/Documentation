GKRTS
=====

.. warning::

   Unreleased, documentation draft


* Tag: |GKRTS_tag|
* Identifier: |GKRTS_identifier|
* Commit: |GKRTS_commit|

.. raw:: html

   <iframe width="696" height="391" src="https://www.youtube.com/embed/" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>


Features
--------

* Combines Gamekit Plugins into a single Real Time strategy game

Useful Links
------------

* `Marketplace <https://www.unrealengine.com/marketplace/en-US/product/91c97c089e004ac69f8fd37566e8b449>`_
* `Playable demo <https://setepenre.itch.io/gkrts>`_
* `Bug Tracker <https://gitlab.com/mischievousgame/gkrts/-/issues>`_
* `Documentation <https://mischievousgame.gitlab.io/gkrts/>`_
* `Youtube Channel <https://www.youtube.com/@mischievousgame>`_
* `Discord Server <https://discord.gg/eqb4Egv9b3>`_
* `Patreon <https://www.patreon.com/setepenre>`_
* `Twitter <https://twitter.com/SetepenreKit>`_


Getting Started
---------------

.. toctree::

   Welcome/Installations
   Welcome/GettingStarted
   Welcome/FrequentQuestions
   Welcome/Changelog


API
---

.. toctree::

   api

Indices and tables
~~~~~~~~~~~~~~~~~~

* :ref:`genindex`
* :ref:`modindex`
* :ref:`search`
